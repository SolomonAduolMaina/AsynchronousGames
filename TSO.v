...
Definition local_buffers := thread_id -> list (nat * nat * Z). (* address, offset, value *)
...
Definition queue (id : thread_id) (val : nat * nat * Z) (local : local_buffers) : local_buffers :=
...
Fixpoint contains_buffered_write (loc : nat * nat)  (l : list (nat * nat * Z)) :=
  match l with
    | nil => false
...
(* A program is a 4-tuple (buf_size, init, s1, s2)
  ...
  2. init : (list (nat * (list Z))) denoting array variables with respective size and inital values,
