...
Definition M_of_tensor (P Q: AsynchronousGame) :=
{i : ((I P) * (I Q)) & (sum unit (sum (N P (fst i)) (N Q (snd i)))) }.

Inductive leq_tensor (P Q : AsynchronousGame) :
(M_of_tensor P Q) -> (M_of_tensor P Q) -> Prop :=
...
Inductive incompatible_tensor (E F : AsynchronousGame) :
(M_of_tensor E F) -> (M_of_tensor E F) -> Prop :=
...
Fixpoint cast_tensor_to_left E F (l : Position (M_of_tensor E F)) : Position (M E) :=
match l with
| nil => nil
...
Fixpoint cast_tensor_to_right E F (l : Position (M_of_tensor E F)) : Position (M F) :=
match l with
| nil => nil
| (existT _ (i,j) (inl tt)) :: xs
...
Definition tensor_move_is_inl (A B : AsynchronousGame) 
(m : (M_of_tensor A B)) : Prop :=
match m with
...
Definition tensor_move_is_inr (A B : AsynchronousGame) 
(m : (M_of_tensor A B)) : Prop :=
match m with
| existT _ _ (inl tt) => True
...
(A B : AsynchronousGame) 
(f : InfinitePosition (M_of_tensor A B))
...
Definition infinite_payoff_left_finite (A B : AsynchronousGame) 
(f : InfinitePosition (M_of_tensor A B) ) 
(inf : bool) :=
exists g l,
...
Definition infinite_payoff_both_infinite (A B : AsynchronousGame) 
(f : InfinitePosition (M_of_tensor A B) ) 
(inf : bool) :=
if inf then
exists g g',
...
(positive1 : positive_or_negative G = true)
(positive2 : positive_or_negative H = true)
: AsynchronousGame :=
        {| 
            I := (I G) * (I H);
            N x := sum (N G (fst x)) (N H (snd x));
            leq := leq_tensor G H;

            incompatible := incompatible_tensor G H;
            ideal m := match m with
                      ...
                      end) (ideal G (existT _ i (inr m)))
                      | existT _ (i, j) (inr (inr m)) =>
                      map (fun x => match x with
                                    | existT _ j (inl tt) => existT _ (i, j) (inl tt)
                                    | existT _ j (inr m) => existT _ (i, j) (inr (inr m))
                      end) (ideal H (existT _ j (inr m)))
                      end;

            polarity m := match m with
                          | existT _ (i,j) (inl ll) => true
                          | existT _ (i,j) (inr (inl m)) => (polarity G) (existT _ i (inr m))
                          | existT _ (i,j) (inr (inr m)) => (polarity H) (existT _ j (inr m))
                          end;
            finite_payoff_position l :=
              (match l with
                | nil => (-1)%Z
                | _ => let left := finite_payoff_position G (cast_tensor_to_left _ _ l) in
                       let right := finite_payoff_position H (cast_tensor_to_right _ _ l) in
            ...
            finite_payoff_walk w :=
              ...
              let left := finite_payoff_walk G ((fp, fm), (sp, sm)) in
              ...
              let right := finite_payoff_walk H ((fp, fm), (sp, sm)) in
              tensor_nat left right;
            infinite_payoff f inf := 
              (infinite_payoff_right_finite G H f inf) \/
              (infinite_payoff_left_finite G H f inf) \/
              (infinite_payoff_both_infinite G H f inf);
...
Program Definition tensor (G H: AsynchronousGame) : AsynchronousGame :=
match positive_or_negative G, positive_or_negative H with
| true, true => asynchronous_game_tensor_positive G H eq_refl eq_refl
| true, false => asynchronous_game_tensor_right G H eq_refl eq_refl
| false, true => asynchronous_game_tensor_left G H eq_refl eq_refl
| false, false => asynchronous_game_tensor_negative G H eq_refl eq_refl
end.
